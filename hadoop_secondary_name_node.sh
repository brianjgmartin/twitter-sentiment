...
# to variable "HADOOP_SECONDARY_NODE_NAME_INST_ID"
HADOOP_SECONDARY_NODE_NAME_INST_ID="$(aws ec2 run-instances \
...
# ${HADOOP_SECONDARY_NODE_NAME_INST_ID}
aws ec2 create-tags \
--resources ${HADOOP_SECONDARY_NODE_NAME_INST_ID} \
--tag Key=Name,Value=HadoopSecondaryNameNode
sleep 60
...
HDSNN_INETADRS="$(aws ec2 describe-instances \
	--filters 'Name=tag:Name,Values=HadoopSecondaryNameNode' \
	--output text \
	--query 'Reservations[*].Instances[*].PrivateIpAddress')"

# Save Public DNS of instance to Variable
HDSNN_PUBDNS="$(aws ec2 describe-instances \
	--filters 'Name=tag:Name,Values=HadoopSecondaryNameNode' \
	--output text \
	--query 'Reservations[*].Instances[*].PublicDnsName')"

#ssh Into HadoopSecondaryNameNode and configure Instance 
ssh -i ~/.ssh/irishkey.pem ubuntu@${PUB_IP_ADRS_HDSNN} bash -c "'
sudo hostname $HDSNN_PUBDNS 
sudo sed -i 's/127.0.0.1/$HDSNN_INETADRS/g' /etc/hosts
sudo sed -i 's/localhost/$HDSNN_PUBDNS/g' /etc/hosts
